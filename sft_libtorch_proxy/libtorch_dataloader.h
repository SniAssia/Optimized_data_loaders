...
struct BenchmarkStats {
    ...
    std::atomic<int64_t> total_token_slots{0};   // padded tokens = proxy work units
    std::atomic<int64_t> total_real_tokens{0};   // non-pad tokens (constant across methods)
    ...
    void add_batch(double collate_ms, double h2d_ms,
                   ...
                   int64_t vram_bytes, int64_t real_tokens = 0) {
    ...
    void print_report() const {
        ...
        // ── PROXY TRAINING TIME ─────────────────────────────
        // Estimates one-epoch training time WITHOUT training, assuming step time
        // scales with the padded tokens the GPU processes: proxy = alpha * padded
        // + beta * batches. alpha=1, beta=0 (defaults) => padded-token units,
        // directly comparable to the category-scheduler pipeline. Override with
        // env PROXY_ALPHA / PROXY_BETA to calibrate to seconds.
        {
            ...
            double pad_pct = padded > 0 ? 100.0 * (padded - real) / padded : 0.0;
...
struct CollateFunction : public torch::data::transforms::BatchTransform
        ...
        std::vector<InstructionDataset::Item> items) const
    {
        ...
        g_stats.add_batch(collate_ms, 0.0,
                          B, batch_max_len, pad_pct, vram_bytes, total_real);
